...
until [ $choice != 'q' -a $choice != 'Q' -a $choice != 'quit' -a $choice != 'Quit' ]&& fdisk -l | grep -o "^磁盘 /dev/[sh]d[a-z]"|grep $choice; 
do
	if [ $choice == 'q' -o $choice == 'Q' -o $choice == 'quit' -o $choice == 'Quit' ]; 
	then 
		echo "程序已退出"
		exit 5
	else
...
if [ $choiceyn == 'N' -o $choiceyn == 'n' ];
then
	echo "程序已退出"
	exit 5
elif [ $choiceyn == 'Y' -o $choiceyn == 'y' ];
then
	echo "正在初始化....请稍后"
